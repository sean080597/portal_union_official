                ...
                <tbody>
                    <tr v-for="(item, index) in ls_evaluation.data" :key="index">
                        ...
                        <td class="text-center">
                            <span class="badge badge-pill badge-success" v-if="item.count_done == ls_evaluation.total_classrooms">Hoàn thành</span>
                            <span class="badge badge-pill badge-warning" v-else>Chưa xong</span>
                        </td>
                        <td class="text-center">{{ item.count_done }}/{{ ls_evaluation.total_classrooms }}</td>
                        <td class="text-center">
                            <router-link :to="'/evaluate-faculty-detail/'+faculty_id+'?y='+item.updated_at" class="text-primary">
    ...
    methods: {
        loadLsEvaluation(){
            ...
            if(this.$gate.isStudentsPagePassed()){
                axios.get('/api/getStatisticFacultyDashboard/' + this.faculty_id).then(({data}) => (
