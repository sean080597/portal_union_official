                ...
                <tbody>
                    <tr v-for="(item, index) in records.data" :key="index">
                        ...
                        <td class="text-center">
                            <span class="badge badge-pill badge-success" v-if="item.evaluated == item.total">Hoàn thành</span>
                            <span class="badge badge-pill badge-warning" v-else>Chưa xong</span>
                        </td>
                        <td class="text-center">{{ item.evaluated }}/{{ item.total }}</td>
                        <td class="text-center">
                            <router-link :to="'/report-classroom/'+item.class_room_id" class="text-primary">
...
export default {
    ...
    methods:{
        getRecords(){
            axios.get('/api/getReportByFaculty/' + this.fac_id)
            .then(({data}) => (
                this.records = data
            ))
        }
