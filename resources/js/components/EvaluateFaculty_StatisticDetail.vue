                ...
                <tbody>
                    <tr v-for="(item, index) in ls_evaluation.data" :key="index">
                        ...
                        <td class="text-center">
                            <span class="badge badge-pill badge-success" v-if="item.evaluated == ls_evaluation.total_classrooms">Hoàn thành</span>
                            <span class="badge badge-pill badge-warning" v-else>Chưa xong</span>
                        </td>
                        <td class="text-center">{{ item.evaluated }}/{{ ls_evaluation.total_classrooms }}</td>
                        <td class="text-center">
                            <router-link :to="'/evaluate-classroom-detail/'+item.class_room_id+'?y='+item.updated_at" class="text-primary">
    ...
    methods: {
        loadLsEvaluation(){
            ...
            if(this.$gate.isStudentsPagePassed()){
                axios.get('/api/getStatisticFacultyDetail/' + this.faculty_id + '?y=' + this.year).then(({data}) => (
